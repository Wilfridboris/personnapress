...
export async function proxy(request: NextRequest) {
  ...
  if (!isPublicRoute) {
    ...
    try {
      ...
      if (payload.onboarding_step != null && (payload.onboarding_step as number) >= 1) {
...
export const config = {
  matcher: ["/((?!_next/static|_next/image|favicon.ico|images/|icons/|fonts/|.*\\.(?:png|jpg|jpeg|gif|svg|ico|webp|woff2?|ttf|eot)).*)"],
};
